...
namespace waxdao {  
  ...
  struct [[eosio::table]] global {
    ...
    EOSLIB_SERIALIZE(global, (maximum_lock_seconds)
                     (minimum_lock_seconds)
                     (total_farms_created)
                     (total_incentives_created)
                     (maximum_incentives_per_farm)
                     (minimum_reward_duration)
                     (maximum_reward_duration)
                     (farm_price_in_wax)
                     (admin_wallets)
  ...
  struct [[eosio::table]] paymethods {
    ...
    uint128_t by_sym_contract() const { return mix64to128( token_symbol.get_symbol().code().raw(), token_symbol.get_contract().value ); }
  };
  using paymethods_table = eosio::multi_index<"paymethods"_n, paymethods,
    eosio::indexed_by<"symcontract"_n, eosio::const_mem_fun<paymethods, uint128_t, &paymethods::by_sym_contract>>
  >;  
}